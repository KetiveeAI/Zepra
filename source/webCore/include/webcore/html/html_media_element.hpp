...
/**
 * @brief HTMLVideoElement - video playback
 *
 * Extends HTMLMediaElement with video-specific properties.
 */
...
public:
    ...
    /// Intrinsic video height
    unsigned int videoHeight() const;

    /// Poster image URL
    std::string poster() const;
    void setPoster(const std::string& poster);

    /// Picture-in-picture disabled
    bool disablePictureInPicture() const;
    void setDisablePictureInPicture(bool disabled);

    /// Plays inline (not fullscreen) on mobile
    bool playsInline() const;
    void setPlaysInline(bool inline_);
 ...
 *
 * Extends HTMLMediaElement with audio-specific properties.
 */
