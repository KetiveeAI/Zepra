...
inline bool Value::isNumber() const {
    // A real number is:
    // 1. Not a NaN pattern at all (regular double), OR
    // 2. A NaN pattern but with tag bits = 0 (actual IEEE NaN)
    if ((bits_ & QNAN_MASK) != QNAN_MASK) {
        // Not our tagged NaN pattern - it's a regular double
        return true;
    }
    // It has the NaN pattern - check if it's a tagged value or actual NaN
    // Our tagged values have non-zero tag bits (TAG_OBJECT, TAG_STRING, etc.)
    // An actual IEEE NaN would have tag bits = 0
    return (bits_ & TAG_MASK) == 0;
}
