...
// DOM Inspector
void DeveloperTools::setDOMTree(std::shared_ptr<DOMNode> root) {
    domTree = root;
    std::cout << "🌳 DOM Tree updated" << std::endl;
}

std::shared_ptr<DOMNode> DeveloperTools::getDOMTree() const {
    return domTree;
}

std::shared_ptr<DOMNode> DeveloperTools::getNodeById(int nodeId) const {
    // Recursive search through DOM tree
    std::function<std::shared_ptr<DOMNode>(std::shared_ptr<DOMNode>)> search = 
        [&](std::shared_ptr<DOMNode> node) -> std::shared_ptr<DOMNode> {
